...
## 📜 Описание
**Mutant Gift Bot** — это автоматизированный бот для игры [@mutant_gifts_bot](https://t.me/mutant_gifts_bot?start=_tgr_uI1FOb9iODEy). Поддерживает многопоточность, интеграцию прокси и автоматическое управление игрой.
...
|| **AUTO_MUTATION**         | True                 | Автоматическая мутация персонажей                       |
|| **AUTO_BATTLE**           | True                 | Автоматические бои                                    |
|| **AUTO_UPGRADE**          | True                 | Автоматическое улучшение персонажей               |
|| **AUTO_DISENCHANT**       | True                 | Автоматическое распыление карточек                |
|| **AUTO_REFILL_ENERGY**    | True                 | Автоматическое восстановление энергии          |
|| **MIN_COINS_BALANCE**     | 0                    | Минимальный баланс монет для сохранения          |
|| **DISENCHANT_RARITIES**   | "Common,Uncommon"    | Редкости карточек для распыления (через запятую)     |
|| **REFILL_ENERGY_TYPE**    | "ranked"             | Тип энергии для восстановления ("ranked", "unranked", "both") |
|| **MAX_ENERGY_REFILLS**    | 1                    | Максимальное кол-во восстановлений каждого типа     |
...
Бот теперь может автоматически восстанавливать энергию за гемы:
- **Обычная энергия** (для обычных боев)
- **Рейтинговая энергия** (для рейтинговых боев)
- **Оба типа** одновременно

💸 **Стоимость восстановления:**
- 1-е восстановление: 60 гемов
- 2-е восстановление: 120 гемов
- 3+ восстановления: 240 гемов

### 🗑️ Автоматическое распыление карточек
Бот автоматически распыляет ненужные карточки низкой редкости:
- Не распыляет закрепленные карточки
- Можно настроить какие редкости распылять

### 🎯 Оптимизированные бои
Бот теперь эффективнее использует энергию:
- Использует **оба типа энергии** для боев
- Не засыпает, если есть любая энергия
- Продолжает цикл до полного истощения
